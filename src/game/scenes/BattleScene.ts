...
export class BattleScene extends Scene {
    ...
    /** Landed pad/turret/rock sprites (sprite-landing): pooled images synced
    ...
    create(): void {
        ...
        // Landed crate face-plates (sprite-landing): one 16px medallion per
        // block at integer 2x, deterministic variant, above the tiled base.
        ...
        // Landed sprites (sprite-landing): fixed-position pad/turret bodies
        // under the dyn overlays (rings, barrels, arcs stay vector); rock
        // pool synced per frame to live W1 telegraphs in drawDynamic.
        this.padKeys = [];
        this.padImgs = this.match.padSnapshots.map((pad) => {
            const key = padSpriteKey(pad.kind, pad.active);
            this.padKeys.push(key);
            return this.add.image(AX + pad.x, AY + pad.y, key).setScale(2).setDepth(1.6);
        });
    ...
    private drawDynamic(snaps: RobotSnapshot[]): void {
        ...
        // Powerup pads: landed kind sprites (sprite-landing) + team-neutral
        ...
        // Map turrets: landed sprite bodies (sprite-landing, variant per
        // turret index, never circles like the chassis, never rings like
        // the pads). Disabled = gray wash, owned = team-color wash + live
        ...
        if (!this.resultsShown) {
            ...
            // Landed asteroid rocks (sprite-landing): one pooled sprite per
            // live telegraph, deterministic variant per slot, integer 2x
            // (128px in the r=70 blast circle). Rings above stay the dodge
            // read; extra telegraphs beyond the pool keep rings only.
